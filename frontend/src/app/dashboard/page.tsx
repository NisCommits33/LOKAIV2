...
export default function DashboardPage() {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8 max-w-[1400px] mx-auto pb-12">
      {/* Welcome section */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium pb-2">
            Continue your organizational learning journey.
          </p>
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="capitalize bg-slate-100 text-slate-600 border-none font-bold text-[10px] uppercase tracking-widest px-2.5 py-0.5">
              {dbUser?.role?.replace("_", " ") || "Public"}
            </Badge>
            {dbUser?.verification_status === "verified" && (
              <Badge className="bg-emerald-50 text-emerald-700 border-none font-bold text-[10px] uppercase tracking-widest px-2.5 py-0.5 shadow-sm">
                Verified Employee
              </Badge>
            )}
            {dbUser?.department_id && (
              <Badge variant="outline" className="text-indigo-600 border-indigo-200 bg-indigo-50 font-bold text-[10px] uppercase tracking-widest px-2.5 py-0.5 shadow-sm">
      ...
      {loadingStats ? (
                             ...
                             <div className="space-y-4">
                                 {analytics.weak_areas.slice(0, 1).map((area: { category: string, accuracy: number }, idx: number) => (
                                     <div key={idx} className="bg-red-50 p-4 rounded-xl border border-red-100">
                                         <div className="flex items-center justify-between mb-1">
                                             ...
                                             <span className="text-xs font-black text-red-500">{area.accuracy}%</span>
                                         </div>
                                         <p className="text-xs text-red-600 font-medium">Topic: {area.category.replace(/_/g, ' ')}</p>
                                     </div>
                                 ))}
                                 {analytics.weak_areas.length === 0 && (
                                     <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                                         ...
                                         <p className="text-xs text-emerald-600 font-medium">You have no major weak areas right now.</p>
                                     </div>
                                 )}
                             </div>
                             
                             <div className="space-y-3 hidden sm:block">
                                 <p className="text-xs font-bold text-slate-900 line-clamp-1 mb-2">Continue where you left off:</p>
                                 {analytics.recommendations.slice(0, 2).map((rec: { id: string, title: string }) => (
                                     <Link key={rec.id} href={`/dashboard/org-documents/${rec.id}`}>
                                        <div className="group flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg transition-colors border-b border-transparent hover:border-slate-100 cursor-pointer">
                                           <div className="flex items-center gap-3">
                                               <FileText className="h-4 w-4 text-slate-400 shrink-0 group-hover:text-indigo-500" />
                                               <span className="text-xs font-semibold text-slate-700 group-hover:text-indigo-700 line-clamp-1">{rec.title}</span>
                                           </div>
                                           <ArrowRight className="h-3.5 w-3.5 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity transform -translate-x-2 group-hover:translate-x-0" />
                                        </div>
                                     </Link>
                                 ))}
                             </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100">
             <div className="flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
                 <div>
                     <h2 className="text-lg font-bold text-indigo-900 tracking-tight flex items-center gap-2 mb-1">
                        Get Started with LokAI <Target className="h-5 w-5 text-indigo-500 hidden sm:block" />
                     </h2>
                     <p className="text-sm text-indigo-700/80 font-medium max-w-xl">
                 ...
                 <Link href="/dashboard/org-documents" className="shrink-0">
                     <div className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-200/50 px-6 py-3 rounded-xl font-bold text-sm transition-transform active:scale-95 flex items-center justify-center h-12">
      ...
      <div className="space-y-4 pt-4">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Quick Navigation</h3>
            ...
            {features.filter((f) => f.show).map((feature, i) => (
                <motion.div key={i} variants={item}>
                  <Link href={feature.href}>
                    <Card className="shadow-none border border-slate-200 overflow-hidden bg-white hover:border-slate-300 hover:shadow-lg transition-all cursor-pointer group h-full">
